...
with st.sidebar:
    ...
    with st.expander('Expand'):
        ...
        title_text_2 = st.text_input('Enter text', 'SCIENCE')
    ...
    with st.expander('Expand'):
        use_custom_logo = st.checkbox('Use custom logo', value=False, key='use_custom_logo')
        custom_logo = None
        streamlit_logo = False  # Default value
        remove_logo_bg = st.checkbox('Remove background from logo', value=True)

        if use_custom_logo:
            custom_logo = st.file_uploader("Upload your logo", type=["png", "jpg", "jpeg"], key='custom_logo')
        else:
            streamlit_logo = st.checkbox('Add Streamlit logo', value=True, key='streamlit_logo')

        logo_width = st.slider('Logo width', 0, 500, 180, step=10)
...
with st.expander('See Wallpaper with Text', expanded=True):
    ...
    with Image.open(f'{img_path}/wallpaper.png') as img:
        ...
        if custom_logo:
            logo_img = Image.open(custom_logo).convert('RGBA')
            if remove_logo_bg:
                logo_img = remove(logo_img)
            logo_img.thumbnail([sys.maxsize, logo_width], Resampling.LANCZOS)
            img.paste(logo_img, (logo_horizontal_placement, logo_vertical_placement), logo_img)
...
if image_upload:
    ...
    st.image(final_img)
    downloadable_final = convert_image(final_img)
    st.download_button("Download final thumbnail", downloadable_final, "final_thumbnail.png", "image/png")
